...
functions and surfaced through the ``agent_plane_client``
...
(``examples/frontends/terminal.py``, ``ap chat``'s raw-schema
path) keep working without modification.

Used by ``ap chat --tools coding`` and the terminal TUI.
"""
...
# ``ap chat`` ``_load_tool_handler`` path reach into modules
...
def execute_tool(name: str, arguments: dict[str, Any]) -> str:
    ...
    Used by ``examples/frontends/terminal.py`` and ``ap chat``'s
    raw-schema path. New consumers should construct a
    :class:`~agent_plane_client.tools.ToolHandler` via
    :func:`~agent_plane_client.tools.build_tool_handler` against
