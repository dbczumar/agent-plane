...
contract for custom Python tools in agent-plane. The decorator:
...
TOOL_MARKER_ATTR = "_ap_tool_metadata"
...
class ToolMetadata:
    ...
    :param synchronous: If ``True``, the tool runs synchronously —
        the LLM blocks on its result (Phase 1 default). If
        ``False``, the framework dispatches the tool through a
        background workflow and returns a task handle to the LLM
        immediately (Phase 2+).
...
def tool(
    ...
    *,
    ...
    """
    Mark a module-level function as an agent-plane tool.
    ...
    Usage::
        ...
        async def train_model(config: TrainConfig) -> TrainResult:
            \"\"\"Long-running. Returns a task handle.\"\"\"
            ...
    ...
    :param fn: When used as bare ``@tool`` (no parens), the
        ...
        ``@tool(synchronous=False)``.
    :param synchronous: If ``True`` (default), the tool's result
        is returned to the LLM directly. If ``False``, the
        framework spawns a background workflow and returns a task
        handle. Phase 1 only supports ``synchronous=True``;
        ``synchronous=False`` is reserved for Phase 2.
