"""LocalServer — start and manage a local agent-plane server."""
...
class LocalServer:
    """Context manager that starts a local agent-plane server.
    ...
    def __init__(
        ...
        *,
        ...
        self._client: AgentPlaneClient | None = None
    ...
    @property
    def client(self) -> AgentPlaneClient:
    ...
    async def __aenter__(self) -> LocalServer:
        ...
        self._client = AgentPlaneClient(base_url=self.base_url)
        return self
    ...
    def _start(self) -> None:
        ...
        self._tmpdir = tempfile.mkdtemp(prefix="agent-plane-client-")
        db_uri = f"sqlite:///{self._tmpdir}/chat.db"
        art_loc = f"{self._tmpdir}/artifacts"

        # Resolve the agent-plane project root so Alembic can find
        # its migrations directory. Walk up from the agent path or
        # from this file's location looking for agent_plane/cli.py.
    ...
    def _wait_for_ready(self, timeout: float = 15.0) -> None:
        ...
        while time.monotonic() < deadline:
            ...
            try:
                resp = httpx.get(f"{self.base_url}/v1/conversations", timeout=2.0)
