...
Concrete policy-dispatch tests land in Phase 3 (LabelPolicy),
Phase 4 (FunctionPolicy), Phase 7 (PromptPolicy).
"""
...
async def test_evaluate_allows_for_every_phase(
    ...
    for phase in Phase:
        content = "body" if phase in (Phase.INPUT, Phase.OUTPUT) else {"tool": "x"}
...
def test_spec_for_unknown_name_returns_none(
    ...
    return None, not raise. The caller (Phase 8
    `_await_policy_approval`) uses the fallback timeout in
    that case."""
