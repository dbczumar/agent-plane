...
async def test_input_phase_deny(
    ...
    """A LabelPolicy on INPUT with DENY action fires on any
...
async def test_tool_result_allow_with_label_write(
    ...
    """A LabelPolicy tainting integrity on tool_result —
...
async def test_output_phase_deny(
    ...
    policy = LabelPolicy(
        LabelPolicySpec(
            name="redact_output",
            on=[PhaseSelector(phase=Phase.OUTPUT)],
            action=PolicyAction.DENY,
...
async def test_policy_fires_on_multiple_phases(
    ...
    policy = LabelPolicy(
        LabelPolicySpec(
            ...
            on=[
                PhaseSelector(phase=Phase.INPUT),
                PhaseSelector(phase=Phase.OUTPUT),
            ],
            action=PolicyAction.ALLOW,
    ...
    engine._labels.clear()  # noqa: SLF001 — reset test state
    ...
    engine._labels.clear()  # noqa: SLF001
    conversation_store.set_labels(conv.id, {"observed": "false"})
