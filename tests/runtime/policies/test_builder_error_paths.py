...
def test_resolve_bare_path_rejected() -> None:
    ...
    with pytest.raises(ValueError, match="dotted module.attribute"):
...
def test_resolve_missing_module_raises_import_error() -> None:
    ...
    with pytest.raises(ImportError):
        resolve_function_policy(
            _fn_spec("agent_plane_nonexistent_module.handler"),
        )
...
def test_resolve_missing_attribute_raises_attribute_error() -> None:
    ...
    with pytest.raises(AttributeError):
        resolve_function_policy(
            _fn_spec("agent_plane.spec.types.nonexistent_attr"),
        )
...
def test_resolve_non_callable_rejected() -> None:
    ...
    # (`agent_plane.spec.types.DEFAULT_ASK_TIMEOUT` = 30, an int).
    with pytest.raises(ValueError, match="not callable"):
        resolve_function_policy(
            _fn_spec("agent_plane.spec.types.DEFAULT_ASK_TIMEOUT"),
        )
...
def test_resolve_with_none_function_raises() -> None:
    ...
    with pytest.raises(ValueError, match="no function reference"):
