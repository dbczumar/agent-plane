...
def test_handle_message_names_task_id_and_tool_name() -> None:
    ...
    know what to pass to ``check_task`` / ``cancel_task``. Without the
    tool name, the LLM may forget which call the handle corresponds to
    when it issued multiple parallel async calls.
    """
    text = _async_handle_message("tsk_async_42", "train_model")
    assert "tsk_async_42" in text
    assert "train_model" in text
    # Mentions both lifecycle tools so the LLM knows its options —
    # otherwise it tends to assume only one is available.
    assert "check_task" in text
    assert "cancel_task" in text
