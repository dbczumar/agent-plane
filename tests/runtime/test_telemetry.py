"""
Unit tests for the ``agent_plane.runtime.telemetry`` helpers.
...
def test_init_respects_capture_content_flag(
    ...
    """
    ``init`` reads ``AGENT_PLANE_OTEL_CAPTURE_CONTENT`` each call
    ...
    monkeypatch.setenv("AGENT_PLANE_OTEL_CAPTURE_CONTENT", "false")
    telemetry.init()
    assert telemetry.should_capture_content() is False
