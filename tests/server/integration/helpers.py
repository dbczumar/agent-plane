...
Required by Phase 2+ concurrency tests (see
``tests/_adherence/phase2.md`` test-infrastructure section). Lives
here rather than ``conftest.py`` because these are plain helpers,
not pytest fixtures.
"""
...
def events_of_type(
    ...
    """
    Filter a captured-event list to a single event type.

    Convenience helper for the common assertion pattern of
    "extract every ``response.output_text.delta`` and check there
    were N of them".

    :param events: Output of :func:`collect_sse_events`.
    :param event_type: The exact ``event:`` value to match, e.g.
        ``"response.heartbeat"``.
    :returns: New list (does not mutate input) of matching events,
