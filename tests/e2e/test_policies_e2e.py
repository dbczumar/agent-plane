...
from tests.e2e.conftest import upload_agent, poll_until_terminal
...
def policy_gate_agent(http_client: httpx.Client) -> str:
    ...
    return upload_agent(http_client, _E2E_POLICY_GATE_DIR)


@pytest.fixture(scope="session")
def label_gate_agent(http_client: httpx.Client) -> str:
    """Upload the e2e-label-gate fixture and return its name."""
    return upload_agent(http_client, _E2E_LABEL_GATE_DIR)
...
def prompt_policy_agent(http_client: httpx.Client) -> str:
    ...
    return upload_agent(http_client, _E2E_PROMPT_POLICY_DIR)
