"""Unit tests for :class:`agent_plane_client.tools.ToolState`.
...
import pytest
from agent_plane_client.tools import ToolState


@pytest.fixture()
def state_root(tmp_path: Path) -> Path:
    """Per-test root directory for ToolState — isolated via tmp_path."""
    root = tmp_path / "tool_state" / "ag_test"
...
def test_illegal_keys_raise(state: ToolState, bad_key: str) -> None:
    ...
    """
    with pytest.raises(ValueError, match="illegal|non-empty"):
        state.set(bad_key, "v")
