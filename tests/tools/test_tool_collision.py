...
``agent_plane.tools.local.load_local_python_tools``; these tests
...
from agent_plane.tools.local import LocalToolLoadError, load_local_python_tools
...
def test_no_collision_with_unrelated_builtin_loads_fine(
    ...
    tools = load_local_python_tools(
        ...
        tmp_path,
        agent_name="testagent",
        builtin_tool_names=frozenset({"web_search", "code_sandbox", "introspect"}),
    )
    assert len(tools) == 1
