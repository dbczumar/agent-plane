...
def test_decorator_synchronous_default_true() -> None:
    """Bare ``@tool`` (no parens) defaults to synchronous=True."""
    md = get_tool_metadata(_t_strict_default_int)
    assert md is not None
    assert md.synchronous is True


def test_decorator_synchronous_false_opt_in() -> None:
    """``@tool(synchronous=False)`` flips the flag."""
    md = get_tool_metadata(_t_async_mode)
    assert md is not None
    assert md.synchronous is False
